...
export default function AjustesScreen() {
  ...
  const enviarInvitacion = () => {
    if (!invitarEmail.trim() || invitarEquipoId === null) return;
    dispatch({
      type: 'ADD_MIEMBRO',
      payload: { equipoId: invitarEquipoId, email: invitarEmail.trim(), rol: invitarRol },
    });
    setInvitarEmail('');
            ...
            state.equipos.map(eq => (
              <View key={eq.id} style={styles.equipoCard}>
                ...
                {eq.miembros.length === 0 ? (
                  <Text style={styles.emptyMiembros}>Sin miembros aún. Invita personas por correo electrónico.</Text>
                ) : (
                  eq.miembros.map((m, idx) => (
                    <View key={idx} style={styles.miembroRow}>
                      <View style={styles.miembroLeft}>
                        <View style={styles.miembroAvatar}>
                          <Text style={styles.miembroAvatarText}>{m.email[0].toUpperCase()}</Text>
                        </View>
                        <View>
...
const styles = StyleSheet.create({
  ...
  addEquipoBtnText: { fontSize: 12, fontWeight: '500', color: colors.outline },
  ...
  rolChipTextActive: { color: colors['on-primary'], fontWeight: '600' },
  saveBtn: {
    height: 44,
